...
import easyocr
import csv
from rought import return_values
...
OCR_TH = 0.2
harsh = 'D:/ANPR_YOLOV/yolov7'



def database(text):
    text = str(text)
    current_time , locname , Owner = return_values(text)
    information = []
    information.extend([text , locname, Owner])
    information.insert(0 , current_time)
...
model =  torch.hub.load(harsh, 'custom' , 'best.pt', source ='local')
classes = model.names
def main(img_path=None, vid_path=None,vid_out = None):
    ...
    if img_path != None:
        ...
        img_out_name = f"harsh{img_path.split('/')[-1]}"
...
# main(vid_path="runs/Traffic27.mp4",vid_out="vid_1.mp4") ### for custom video

# main(vid_path=0,vid_out="webcam_facemask_result.mp4") #### for webcam

# main(img_path="1.jpg") ## for image